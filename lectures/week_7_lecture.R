...
#best practices: remove backgrounds, redundant labels, borders, reduce colors and special effects, remove bolding, lighten labels, remove lines, direct labels
ggplot(diamonds, aes(x=carat, y=price, color=clarity)) + geom_point(alpha=0.2) + theme_classic()

#straight-line relationship
ggplot(diamonds, aes(x=carat, y=price, color=clarity)) + geom_point(alpha=0.2) + theme_classic() + ggtitle("price by diamond quality") + ylab("price in $") + xlab("carat") + stat_smooth(method = "lm")

#non-straight line relationship (loess)
ggplot(diamonds, aes(x=carat, y=price, color=clarity)) + geom_point(alpha=0.2) + theme_classic() + ggtitle("price by diamond quality") + ylab("price in $") + xlab("carat") + stat_smooth(method = "loess")
...
ggplot(iris, 
  ...
  scale_fill_brewer(palette = "Set2") + theme(plot.title = element_text(hjust = 0.5, size = 15, face = "bold"))
#how did we know the name of the palette is "Set2"? From this list:
display.brewer.all(colorblindFriendly = TRUE)

#HOW TO MAKE YOUR TITLES CENTERED!
...
plot.diamonds <- ggplot(diamonds, aes(clarity, fill = cut)) + 
  geom_bar() +
  theme(axis.text.x = element_text(angle=70, vjust=0.5)) + theme_classic()
#plot.diamonds
...
plot.iris <- ggplot(data=iris, aes(x=Sepal.Length, y=Petal.Length, fill=Species)) +
  geom_point(size=3, alpha=0.7, shape=21) + theme_classic()
#plot.iris
